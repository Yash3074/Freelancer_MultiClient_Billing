...
class App {
  ...
  loadProfileForm() {
    ...
    document.getElementById('profileCountry').value = user.country || 'India';
    ...
    document.getElementById('profileAvatar').textContent = this.getInitials(user.name);
  }
  ...
  bindAuthEvents() {
    ...
    forgotForm?.addEventListener('submit', (e) => {
      e.preventDefault();
      try {
        const msg = this.auth.forgotPassword(document.getElementById('forgotEmail').value);
        Toast.info(msg);
      } catch (err) {
        Toast.error(err.message);
      }
  ...
  bindAppEvents() {
    ...
    document.getElementById('addItemBtn').onclick = () => this.addLineItem();
    ...
    invoiceForm.onsubmit = (e) => {
      ...
      document.querySelectorAll('.line-item-row').forEach(row => {
        ...
        if (desc && qty && rate) items.push({ description: desc, quantity: qty, rate: rate });
      });

      if (items.length === 0) return Toast.error('Please enter at least one line item.');

      const status = document.getElementById('invoiceInitialStatus').value;
      const initialPaid = parseFloat(document.getElementById('invoiceInitialPaid').value) || 0;
    ...
    document.getElementById('invoiceClientSelect')?.addEventListener('change', () => this.updateInvoicePreview());
    ...
    document.getElementById('profileForm')?.addEventListener('submit', (e) => {
      ...
      try {
        ...
        document.getElementById('navUserName').textContent = updates.name;
        document.getElementById('navUserAvatar').textContent = this.getInitials(updates.name);
        this.loadProfileForm();
      } catch (err) {
        Toast.error(err.message);
      }
  ...
  resetClientForm() {
    ...
    document.getElementById('btnSaveClient').textContent = 'Save Client';
    document.getElementById('btnCancelClientEdit').style.display = 'none';
    document.getElementById('clientForm').reset();
    document.getElementById('clientCountry').value = 'India';
  }
  ...
  addLineItem() {
    ...
    const updatePreview = () => this.updateInvoicePreview();
  ...
  updateInvoicePreview() {
    const clientId = document.getElementById('invoiceClientSelect')?.value;
    const client = clientId ? this.clientController.getClientById(clientId) : null;
    ...
    const isInternational = client ? (client.currency !== 'INR' || client.country !== 'India') : false;
    ...
    document.getElementById('previewSubtotal').textContent = subtotal > 0 ? fmt(subtotal) : '—';
    document.getElementById('previewGST').textContent = subtotal > 0 ? `${fmt(gstInfo.gstAmount)} (${gstInfo.rate}%)` : '—';
    document.getElementById('previewTotal').textContent = subtotal > 0 ? fmt(total) : '—';
  }
  ...
  renderClientsList() {
    ...
    clients.forEach(c => {
      ...
      card.innerHTML = `
        ...
        <p style="margin-top: 0.75rem; font-size: 0.88rem; color: var(--text-muted);"><strong style="color: var(--text-main);">Address:</strong> ${c.address}</p>
      `;

      card.querySelector('.btn-delete').onclick = () => {
        this.clientController.deleteClient(c.id);
      ...
      card.querySelector('.btn-edit').onclick = () => {
        ...
        document.getElementById('clientGSTIN').value = c.gstin === 'N/A' ? '' : c.gstin;
        document.getElementById('clientAddress').value = c.address;

        document.getElementById('clientFormTitle').textContent = `Edit Client — ${c.name}`;
        document.getElementById('btnSaveClient').textContent = 'Update Client';
  ...
  renderInvoicesList() {
    ...
    invoices.forEach(inv => {
      ...
      const badgeClass = inv.status === 'Fully Paid' ? 'badge-paid'
      ...
      card.innerHTML = `
        ...
        <p style="margin-top: 0.75rem; font-size: 0.88rem; color: var(--text-muted);">
          <strong style="color: var(--text-main);">Collected:</strong>
          ${inv.currency} ${inv.amountPaid.toLocaleString()} (₹${inv.amountPaidINR.toLocaleString('en-IN')})
        </p>
        ...
        <div class="payment-control-box">
          ...
          <div class="payment-controls">
            ...
            <input type="number" class="partial-amount-input" data-id="${inv.id}"
                   placeholder="Amount Paid" value="${inv.amountPaid}"
                   style="display: ${inv.status === 'Partially Paid' ? 'inline-block' : 'none'};">
            <button class="btn-gradient btn-update-payment" data-id="${inv.id}" style="padding: 0.45rem 1rem; font-size: 0.85rem;">Save Status</button>
          </div>
        </div>

        <div class="card-actions">
          <button class="btn-print" data-id="${inv.id}">Print Invoice</button>
          <button class="btn-delete" data-id="${inv.id}">Delete</button>
        </div>
      `;

      const statusSelect = card.querySelector('.status-select');
      const partialInput = card.querySelector('.partial-amount-input');

      statusSelect.onchange = (e) => {
        partialInput.style.display = e.target.value === 'Partially Paid' ? 'inline-block' : 'none';
      };

      card.querySelector('.btn-update-payment').onclick = () => {
        this.invoiceController.updatePaymentStatus(inv.id, statusSelect.value, parseFloat(partialInput.value) || 0);
      ...
      card.querySelector('.btn-print').onclick = () => this.printInvoice(inv);
  ...
  updateDashboard() {
    ...
    document.getElementById('rebateNotice').style.display = taxProjection.rebate87A > 0 ? 'block' : 'none';
  }
    ...
    printWin.document.write(`
      <html>
        ...
        <body>
          ...
          ${inv.notes ? `<p style="margin-top: 32px; padding-top: 16px; border-top: 1px solid #e2e8f0; color: #64748b; font-size: 0.9rem;"><strong>Notes:</strong> ${inv.notes}</p>` : ''}
        </body>
      </html>
    `);
