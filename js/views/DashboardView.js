/**
 * DashboardView
 * Responsible for rendering all stat cards, tax slab table,
 * and the Freelancer Tax Benefits section.
 */
export class DashboardView {
   ...
   * @param {import('../models/FreelancerTaxBenefits.js').FreelancerTaxBenefits} freelancerTaxBenefits
   */
  ...
  update() {
    ...
    // ── Stat cards ──────────────────────────────────────────────
    this._setText('dashGrossIncome',
      `₹${this._fmt(grossRealizedINR)}`);

    this._setText('dashTaxLiability',
      `₹${this._fmt(taxProjection.totalTaxLiability)}`);

    this._setText('dashNetIncome',
      `₹${this._fmt(netAfterTax)}`);

    this._setText('dashTotalExpenses',
      `₹${this._fmt(totalExpenses)}`);

    this._setText('dashRealNetIncome',
      `₹${this._fmt(realNet)}`);
    ...
    const professionEligible =
      document.getElementById('freelancerProfession')?.value === 'yes';
    const cashPct = Number(
      document.getElementById('freelancerCashReceipts')?.value || 0
    );
  ...
  _updateFreelancerTaxUI(result) {
    ...
    this._setText('freelancerGrossReceipts',
      `₹${this._fmt(result.grossReceipts)}`);
    this._setText('freelancer44ADALimit',
      `₹${result.receiptLimit.toLocaleString('en-IN')}`);
    this._setText('freelancerPresumptiveIncome',
      `₹${this._fmt(result.presumptiveIncome)}`);
    this._setText('freelancerTaxWithout',
      `₹${this._fmt(result.taxWithout44ADA)}`);
    this._setText('freelancerTaxWith',
      `₹${this._fmt(result.taxWith44ADA)}`);
    this._setText('freelancerTaxSaving',
      `₹${this._fmt(result.estimatedTaxSaving)}`);
